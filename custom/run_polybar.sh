...
then
    ...
    EXTEND=false MONITOR=$m $COMMAND -c "$CFG" bottom >/dev/null 2>&1 &
    EXTEND=false MONITOR=$m $COMMAND -c "$CFG" top >/dev/null 2>&1 &

else

    for m in $(xrandr -q | grep " connected" | cut -d" " -f1); do
        if [ "$m" = "$PRIMARY" ] ; then
            MONITOR=$m $COMMAND -c "$CFG" bottom >/dev/null 2>&1 &
            MONITOR=$m $COMMAND -c "$CFG" top >/dev/null 2>&1 &
            echo
        else
            MONITOR=$m $COMMAND -c "$CFG" bottom2 >/dev/null 2>&1 &
#            MONITOR=$m $COMMAND -c "$CFG" top2 >/dev/null 2>&1 &
            echo
        fi
    done
